...
const labelHelper = (parent, node, _classes, isNode) => {
  let classes;
  const useHtmlLabels = node.useHtmlLabels || mermaid_core.evaluate(mermaid_core.getConfig().flowchart.htmlLabels);
  if (!_classes) {
  ...
  if (node.labelType === "markdown") {
    text = createText1f5f8f92.createText(label, mermaid_core.sanitizeText$1(mermaid_core.decodeEntities(labelText), mermaid_core.getConfig()), {
      useHtmlLabels,
      width: node.width || mermaid_core.getConfig().flowchart.wrappingWidth,
      classes: "markdown-node-label"
    });
  } else {
    text = textNode.appendChild(
      createLabel$1(
        mermaid_core.sanitizeText$1(mermaid_core.decodeEntities(labelText), mermaid_core.getConfig()),
        node.labelStyle,
        false,
        isNode
      )
...
const note = (parent, node) => {
  const useHtmlLabels = node.useHtmlLabels || mermaid_core.getConfig().flowchart.htmlLabels;
  if (!useHtmlLabels) {
...
const class_box = (parent, node) => {
  ...
  node.classData.members.forEach((str) => {
    const parsedInfo = svgDraw2526cba0.parseMember(str);
...
const insertEdgeLabel = (elem, edge) => {
  ...
  const labelElement = edge.labelType === "markdown" ? createText1f5f8f92.createText(elem, edge.label, {
    style: edge.labelStyle,
    useHtmlLabels,
    addSvgBackground: true
  }) : createLabel$1(edge.label, edge.labelStyle);
  mermaid_core.log.info("abc82", edge, edge.labelType);
