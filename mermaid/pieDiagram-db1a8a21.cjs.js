...
function pie() {
  var value = identity,
      ...
      endAngle = path.constant(mermaid_core.tau),
  ...
  function pie(data) {
    var i,
        ...
        da = Math.min(mermaid_core.tau, Math.max(-mermaid_core.tau, endAngle.apply(this, arguments) - a0)),
        a1,
...
const draw = (txt, id, _version, diagObj) => {
  ...
  try {
    ...
    let [outerStrokeWidth] = mermaid_core.parseFontSize(themeVariables.pieOuterStrokeWidth);
    ...
    var labelArcGenerator = arc.arc().innerRadius(radius * textPosition).outerRadius(radius * textPosition);
